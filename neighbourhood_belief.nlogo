...
to setup-neighbourhoods
  ...
  while [length neighbourhoods < number-of-neighbourhoods] [
    let candidate one-of patches with [
      pxcor >= (min-pxcor + length_neighbourhood) and
      pxcor <= (max-pxcor - length_neighbourhood) and
      pycor >= (min-pycor + length_neighbourhood) and
      pycor <= (max-pycor - length_neighbourhood)
    ]
    let hood-patches patches with [
      abs(pxcor - [pxcor] of candidate) <= length_neighbourhood and
      abs(pycor - [pycor] of candidate) <= length_neighbourhood
    ]
    if all? hood-patches [neighbourhood-id = nobody and not member? self common] [
      set neighbourhoods lput candidate neighbourhoods
      ask hood-patches [ set neighbourhood-id candidate ]
    ]
  ]

  let all_colours (range 0 140)
  let neighbourhood_colours n-of number-of-neighbourhoods (filter [colour -> colour != 56] all_colours)

  (foreach (sort neighbourhoods) neighbourhood_colours
  [[neighbourhood neighbourhood_colour] ->
    let hood-patches patches with [neighbourhood-id = neighbourhood]
    ask hood-patches [ set pcolor neighbourhood_colour ]
...
to create-meeting
  ...
  if any? others [
    ...
    if immune? [
      set updated-belief my-belief
    ]
    if (not immune?) [
      if share >= threshold [
        set updated-belief not my-belief
      ]
      if share < threshold [
        set updated-belief my-belief
      ]
...
to update-immunity
  if (belief = true) and (not immune?) [
    let chance immunity-level + confidence-level / 2
    if chance > immunity-threshold [
      set immune? true
      set color green
    ]
  ]
...
to update-opinions-in-hoods
  let hood-seeds remove-duplicates [neighbourhood-id] of patches with [neighbourhood-id != nobody]
  if empty? hood-seeds [ stop ]

  if belief-indiviual-OR-neighbourhood [
    individual-hood-updates hood-seeds
  ]
  if not belief-indiviual-OR-neighbourhood [
    collective-hood-updates hood-seeds
  ]
end

to individual-hood-updates [hood-seeds]
  foreach hood-seeds [ hood ->
    let residents people with [[neighbourhood-id] of home-neighbourhood = hood]
    if any? residents [
      ask residents [
        let my-belief belief
        let others other residents
        if any? others [
          let opposing 0
          ask others [ if belief != my-belief [ set opposing opposing + 1 ] ]
          let share opposing / count others
          let threshold 0.5 + (confidence-level / 200)
          if immune? [ set updated-belief my-belief ]
          if not immune? [
            if share >= threshold [ set updated-belief not my-belief ]
            if share <  threshold [ set updated-belief my-belief ]
          ]
        ]
        if not any? others [ set updated-belief my-belief ]
      ]
      ask residents [
        if updated-belief != belief [
          set belief updated-belief
...
to collective-hood-updates [hood-seeds]
  let collective-threshold 0.5
  foreach hood-seeds [ hood ->
    let residents people with [[neighbourhood-id] of home-neighbourhood = hood]
    if count residents >= 1 [
      let pro count residents with [belief]
      let total count residents
      let pro-share pro / total
      let new-hood-belief (pro-share >= collective-threshold)

      ask residents [
        if immune? [ set updated-belief belief ]
        if not immune? [ set updated-belief new-hood-belief ]
      ]
      ask residents [
        if updated-belief != belief [
          set belief updated-belief
...
to-report pro-count
  report count turtles with [belief = true]
end

to-report anti-count
  report count turtles with [belief = false]
end
@#$#@#$#@
GRAPHICS-WINDOW
210
...
253
495
belief-indiviual-OR-neighbourhood
belief-indiviual-OR-neighbourhood
0
1
-1000
